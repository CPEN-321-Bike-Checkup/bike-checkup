...
export default class MaintenancePrediction extends React.Component {
	...
	render() {
		if (this.state.predictedDates.length > 0) {
			...
			for (let maintIndex = 0; maintIndex < maintenanceList.length; maintIndex++) {

				var predictedMaintDate
					= this.state.predictedDates[maintIndex].toString();

				var maintDescription
					= maintenanceList[maintIndex].description;
